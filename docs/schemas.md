...
The schema is used to produce the opensearch configuration for each index,
but is also used to control how the fields are processed before passing them to
opensearch, and after retrieving results from opensearch.
...
the name of the type, and has the following properties:
 ...
 - `es_config`: the opensearch configuration to be applied to fields using
   this type.
 ...
 - `children`: (optional) - if present, the field contains child fields (ie,
   ...
   - `dynamic`: Arbitrary child fields will be accepted, but how they are
     handled will be configured dynamically according to the opensearch
     configuration.
...
The basic configuration is defined in a `base_opensearch_type.json` file.
Document types are defined in the `opensearch_types` directory, with the
...
[Indexes in opensearch][opensearch-indexes] are defined by files in the `indexes` directory.
These files contain a JSON object with the following keys:

 - `opensearch_types`: An array of the names of the document types allowed in this index.
...
and then passed to opensearch directly.
