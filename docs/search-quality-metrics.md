...
We use explicit judgements based on expert opinion. The relevance judgements are uploaded in 
CSV format to an S3 bucket, which then gets pulled by search-api when the rake task runs.
