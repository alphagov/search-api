class SchemaConfig
  ...
  def initialize(config_path, schema_config_file: "opensearch_schema.yml")
  ...
  def opensearch_settings(_index_name)
    @opensearch_settings ||= opensearch_index["settings"]
  end

  def opensearch_types(index_name)
    index_name = index_name.sub(/[-_]test$/, "")
    @index_schemas.fetch(index_name).opensearch_types
  end

  def opensearch_mappings(index_name)
    index_name = index_name.sub(/[-_]test$/, "")
