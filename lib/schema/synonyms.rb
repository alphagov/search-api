...
private
  ...
  def validate_key!(key)
    unless VALID_KEYS.include?(key)
      raise InvalidSynonymConfig, "Unknown synonym key '#{key}'. Expected one of: #{VALID_KEYS.join(', ')}"
    end
  end

  def validate_synonym_count!(synonyms)
    if synonyms.count > 1
      raise InvalidSynonymConfig, <<~MESSAGE
  ...
  def validate_term!(term, existing_terms, key)
    if existing_terms.include?(term)
      raise InvalidSynonymConfig, "Synonym '#{term}' already defined for '#{key}'"
    end
  end
  ...
  private
    ...
    def validate_values!(synonym, values)
      if values.nil? || values.strip.empty?
        raise InvalidSynonymConfig, "Synonym '#{synonym}' has no definition"
      end
    end
    ...
    def validate_terms!(terms)
      terms.split(",").map(&:strip).each do |term|
        if unique_terms.include?(term)
          raise InvalidSynonymConfig, "Synonym '#{term}' already defined for '#{synonym_type}'"
        end
