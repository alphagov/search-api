...
private
  ...
  def order
    ...
    unless ALLOWED_SORT_FIELDS.include?(field)
      @errors << %Q{"#{field}" is not a valid sort field}
      return nil
    end
    return [field, dir]
  end
    ...
    @params.each do |key, value|
      if (m = key.match(/\Afilter_(.*)/))
        ...
        if ALLOWED_FILTER_FIELDS.include? field
          ...
          @errors << %Q{"#{field}" is not a valid filter field}
        end
        @used_params << key
      end
    end
    ...
    @params.each do |key, value|
      if (m = key.match(/\Afacet_(.*)/))
        ...
        if ALLOWED_FACET_FIELDS.include? field
          count = parse_positive_integer(value, %Q{facet "#{field}"})
          unless count.nil?
            facets[field] = count
          end
        else
          @errors << %Q{"#{field}" is not a valid facet field}
        end
        @used_params << key
      end
    end
  ...
  def integer_param(param_name, default)
    ...
    unless value.nil?
      value = parse_positive_integer(value, %Q{parameter "#{param_name}"})
    end
    if value.nil?
      return default
    end
  ...
  def parse_positive_integer(value, description)
    begin
      ...
      @errors << %Q{Invalid value "#{value}" for #{description} (expected positive integer)}
      return nil
    end
    if result < 0
      @errors << %Q{Invalid negative value "#{value}" for #{description} (expected positive integer)}
      return nil
    end
    result
  end
