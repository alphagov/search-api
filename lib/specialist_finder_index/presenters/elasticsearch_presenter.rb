module SpecialistFinderIndex
  class ElasticsearchPresenter
    include GovukIndex::ElasticsearchIdentity
    ...
    def document
      ...
      }.merge(specialist_fields).reject { |_, v| v.nil? }
    end

    def specialist_fields
      {
  ...
  private
    ...
    def indexable
      GovukIndex::IndexableContentPresenter.new(
        format: common_fields.format,
    ...
    def common_fields
      @common_fields ||= GovukIndex::CommonFieldsPresenter.new(payload)
    end

    def details
      @details ||= GovukIndex::DetailsPresenter.new(details: payload["details"], format: common_fields.format)
    end

    def parts
      @parts ||= GovukIndex::PartsPresenter.new(parts: payload["details"].fetch("parts", []))
    end

    def expanded_links
      @expanded_links ||= GovukIndex::ExpandedLinksPresenter.new(payload["expanded_links"])
    end
