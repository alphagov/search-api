...
ssh -i /tmp/concourse_ssh_key -o StrictHostKeyChecking=no "ubuntu@${instance_ip}" << EOF
  ...
  mkdir tmp
  set +x
  echo "running bundle exec rake learn_to_rank:fetch_bigquery_export[BIGQUERY_CREDENTIALS,bigquery]"
  ...
  set +x
  echo "running bundle exec rake learn_to_rank:fetch_bigquery_export[BIGQUERY_CREDENTIALS,bigquery_small,3500]"
