...
ssh -i /tmp/concourse_ssh_key -o StrictHostKeyChecking=no "ubuntu@${instance_ip}" << EOF
  ...
  mkdir tmp
  set +x
  echo "running bundle exec rake learn_to_rank:fetch_bigquery_export[bigquery]"
  ...
  set +x
  echo "running bundle exec rake learn_to_rank:fetch_bigquery_export[bigquery_small,3500]"
  \$docker bundle exec rake 'learn_to_rank:fetch_bigquery_export[bigquery_small,3500]'
  set -x
