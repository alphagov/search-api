...
RSpec.describe "locales" do
  ...
  it "indexes pages missing a locale" do
    ...
    allow(GovukIndex::IndexableFormats).to receive(:indexable_formats).and_return("taxon" => :all)
  ...
  it "indexes Welsh pages for the hmrc_contact document type" do
    ...
    allow(GovukIndex::IndexableFormats).to receive(:indexable_formats).and_return("hmrc_contact" => :all)
    @queue.publish(random_example.to_json, content_type: "application/json")
