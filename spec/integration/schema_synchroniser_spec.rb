...
RSpec.describe SchemaSynchroniser do
  ...
  let(:schema_config) { SearchConfig.default_instance.schema_config }
  let(:mappings) { schema_config.elasticsearch_mappings(index_name) }
  ...
  it "returns an error if the mappings are invalid" do
    ...
    synchroniser = described_class.new(index_group)
    result = synchroniser.call(mappings)
