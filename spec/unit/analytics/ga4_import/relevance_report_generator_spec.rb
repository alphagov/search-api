...
RSpec.describe Analytics::Ga4Import::RelevanceReportGenerator do
  ...
  let(:relevancy_calculator) { instance_double(Analytics::Ga4Import::OpenSearchRelevancySerialiser) }
  ...
  before do
    ...
    allow(Analytics::Ga4Import::OpenSearchRelevancySerialiser).to receive(:new).and_return(relevancy_calculator)
    allow(relevancy_calculator).to receive(:relevance).and_return(['{"index": {"_index": "page-traffic", "_id": 1}}', '{"path": "/a/b/c", "views": 100}'])
  end

  it "calls DataFetcher, PageViewConsolidator, and OpenSearchRelevancySerialiser" do
    ...
    expect(Analytics::Ga4Import::OpenSearchRelevancySerialiser).to have_received(:new).with(:consolidated_page_views)
