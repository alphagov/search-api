...
class ClientTest < MiniTest::Unit::TestCase
  ...
  def test_logs_error_body
    ...
    Logging.logger[LegacyClient::Client].expects(:error)
      .with(regexp_matches(/STUFF AND THINGS BROKE/))

    assert_raises RestClient::InternalServerError do
      LegacyClient::Client.new("http://localhost/").get("")
    end
  end

  def test_timeout
    # Test that LegacyClient::Client accepts a timeout option and passes it on
    # to RestClient
    RestClient::Request.expects(:execute).with(has_entries(timeout: 10))
    LegacyClient::Client.new("http://localhost/", timeout: 10).get("")
  end

  def test_open_timeout
    # Test that LegacyClient::Client accepts an open_timeout option and passes
    # it on to RestClient
    RestClient::Request.expects(:execute).with(has_entries(open_timeout: 10))
    LegacyClient::Client.new("http://localhost/", open_timeout: 10).get("")
  end
end
